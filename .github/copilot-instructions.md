...
- ❌ Import `infrastructure/*` from components or pages — they get data via
  `presentation/hooks/*` only; never call the API client / `httpClient` from UI.
- ❌ Point dependencies outward, or skip a layer (always
  hook → use-case → repository port → impl → client).
- ❌ Put business logic (fetching, DTO→entity mapping, pagination/filter logic) in
  components — that belongs in use cases, mappers, repositories.
